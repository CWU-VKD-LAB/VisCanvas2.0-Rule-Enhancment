...
void OpenGLForm::COpenGL::generateRulesDNS()
{
	...
	thread.updateLoadingText("Michael's Rules");

	rules.push_back("Michael's Rule Generation\n");

	rules = doMichaelRuleGen(file->getFilePath(), rules, 1);

	rules.push_back("Eli's Rule Generation\n");
	...
	for (int i = 0; i < precThreshs.size(); i++)
	{
		...
		thread.updateLoadingText("Eli's Rules" + prec + "% precision");
