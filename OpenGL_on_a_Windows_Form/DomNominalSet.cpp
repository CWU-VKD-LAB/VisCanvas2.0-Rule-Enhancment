...
pair<vector<string>, vector<DNSRule>> DomNominalSet::MTBRGSequential(double precisionThresh, vector<vector<int>>groups, int targetClass)
{
	...
	for (int i = 0; i < allGroupRules.size(); i++)
	{
		...
		for (int j = 0; j < allGroupRules.size(); j++)
		{
			...
			toReturn.push_back("overlap between rule " + to_string(ruleOneCount) + " and " + to_string(ruleTwoCount) + ": " + to_string(overlapPercentage) + "%\n");
...
//joins two rules on a particular clause. For example, If I have two rules: R1: {x1 = 2, x4 = 3} and R2: {x1 = 2, x5 = 7},, and my join index is 0, then a new rule
//is made from those two rules: R {X1 = 2 AND (X4 = 3 || X5 = 7)}
DNSRule DomNominalSet::joinRulesAt(int joinIndex, DNSRule rule1, DNSRule rule2, int casesInTarget)
{
	DNSRule combinedRule;
